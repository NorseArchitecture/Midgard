...
readonly record struct LocationInfo(string FilePath, TextSpan Span, LinePositionSpan LineSpan)
{
	...
	/// <summary>
	///     The symbol's first source location, or <see cref="None" /> when the symbol has none in source (a
	///     referenced-assembly type).
	/// </summary>
	public static LocationInfo FromSymbol(ISymbol symbol) =>
		symbol.Locations.FirstOrDefault(l => l.IsInSource) is { } location ?
...
/// <summary>
///     One defined value of an enum-typed scalar member, carried so the writer/reader (a later task) can emit a
///     build-time name↔value table with no <c>Enum.Parse</c>/reflection at runtime.
/// </summary>
...
/// </summary>
readonly record struct ControllerShapeResult(
	EquatableArray<ShapeModel> Shapes,

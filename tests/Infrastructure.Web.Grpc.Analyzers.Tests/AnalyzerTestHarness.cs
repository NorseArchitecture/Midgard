...
static class AnalyzerTestHarness
{
	...
	public static CSharpCompilation CreateCompilation(string assemblyName, MetadataReference[] extraReferences,
		...
		DiagnosticAnalyzer analyzer, string assemblyName, MetadataReference[] extraReferences, params string[] sources)
	{
		...
		var compileErrors = compilation.GetDiagnostics().Where(d => d.Severity == DiagnosticSeverity.Error)
			.ToImmutableArray();
		...
		var withAnalyzers = compilation.WithAnalyzers(
			...
			new CompilationWithAnalyzersOptions(
				...
				onAnalyzerException: (Action<Exception, DiagnosticAnalyzer, Diagnostic>?)null,

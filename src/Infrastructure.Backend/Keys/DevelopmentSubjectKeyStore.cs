...
public sealed class DevelopmentSubjectKeyStore(string rootPath) : ISubjectKeyStore, ILookupKeyRing
{
	...
	public async ValueTask<SubjectKeyResult> GetAsync(Guid subjectId, CancellationToken cancellationToken = default)
	{
		...
		if (File.Exists(keyPath))
			return SubjectKeyResult.Available(await File.ReadAllBytesAsync(keyPath, cancellationToken)
				.ConfigureAwait(false));

		var receiptPath = ReceiptPath(subjectId);
		if (File.Exists(receiptPath))
			return SubjectKeyResult.Destroyed(await ReadReceiptAsync(receiptPath, cancellationToken)
				.ConfigureAwait(false));

		var pendingReceiptPath = PendingReceiptPath(subjectId);
		return File.Exists(pendingReceiptPath) ?
			SubjectKeyResult.Destroyed(await ReadReceiptAsync(pendingReceiptPath, cancellationToken)
				.ConfigureAwait(false)) :
	...
	public async ValueTask<byte[]> GetOrCreateAsync(Guid subjectId, CancellationToken cancellationToken = default)
	{
		...
		if (File.Exists(receiptPath))
			throw new KeyDestroyedException(
				await ReadReceiptAsync(receiptPath, cancellationToken).ConfigureAwait(false));

		var pendingReceiptPath = PendingReceiptPath(subjectId);
		if (File.Exists(pendingReceiptPath))
			throw new KeyDestroyedException(await ReadReceiptAsync(pendingReceiptPath, cancellationToken)
				.ConfigureAwait(false));
	...
	public async ValueTask<ErasureReceipt> DestroyAsync(Guid subjectId, CancellationToken cancellationToken = default)
	{
		...
		await File.WriteAllBytesAsync(pendingReceiptPath,
				JsonSerializer.SerializeToUtf8Bytes(document, KeysJsonContext.Default.ReceiptDocument),
				cancellationToken)
			.ConfigureAwait(false);
	...
	void EnsureLookupRing()
	{
		...
		lock (_lookupGate)
		{
			...
			{
				_lookup = JsonSerializer.Deserialize(File.ReadAllBytes(lookupPath),
						KeysJsonContext.Default.LookupDocument) ??
			...
			File.WriteAllBytes(lookupPath,
				JsonSerializer.SerializeToUtf8Bytes(minted, KeysJsonContext.Default.LookupDocument));
			_lookup = minted;
		}
	}

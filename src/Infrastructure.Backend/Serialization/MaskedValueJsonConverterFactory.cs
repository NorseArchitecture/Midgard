	...
	sealed class MaskedValueJsonConverter<T> : JsonConverter<T> where T : struct, IMaskedValue
	{
		public override T Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
			throw new NotSupportedException(
				$"{typeToConvert.Name} is masked-write-only JSON; PII never rehydrates from JSON — parse the wire string at the boundary instead.");

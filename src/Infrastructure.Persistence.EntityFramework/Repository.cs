	...
	where TView : notnull
{
	...
	public async Task<Outcome<TView>> GetAsync(Guid id, CancellationToken cancellationToken = default)
	{
		...
		var view = await QueryById(context, id).Select(_selector).FirstOrDefaultAsync(cancellationToken)
			.ConfigureAwait(false);
		return view is null ?
			Outcome<TView>.Err(ErrorCategory.NotFound) :
			Outcome<TView>.Ok(view);
	}
	...
	public async Task<Outcome<TProjection>> GetAsync<TProjection>(Guid id,
		...
		where TProjection : notnull
	{
		...
		var results = await QueryById(context, id).Select(_selector).Select(projection).Take(1)
			.ToListAsync(cancellationToken).ConfigureAwait(false);
		return results.Count == 0 ?
			Outcome<TProjection>.Err(ErrorCategory.NotFound) :
			Outcome<TProjection>.Ok(results[0]);
	}

	public async Task<Outcome<TView>> FirstAsync(Expression<Func<TView, bool>> predicate,
		CancellationToken cancellationToken = default)
	{
		var context = await factory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false);
		await using var contextDisposable = context.ConfigureAwait(false);
		var view = await Query(context, predicate).Select(_selector).FirstOrDefaultAsync(cancellationToken)
			.ConfigureAwait(false);
		return view is null ?
			Outcome<TView>.Err(ErrorCategory.NotFound) :
			Outcome<TView>.Ok(view);
	...
	public async Task<Outcome<TProjection>> FirstAsync<TProjection>(Expression<Func<TView, bool>> predicate,
		Expression<Func<TView, TProjection>> projection, CancellationToken cancellationToken = default)
		where TProjection : notnull
	{
		var context = await factory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false);
		...
		var results = await Query(context, predicate).Select(_selector).Select(projection).Take(1)
			.ToListAsync(cancellationToken).ConfigureAwait(false);
		return results.Count == 0 ?
			Outcome<TProjection>.Err(ErrorCategory.NotFound) :
			Outcome<TProjection>.Ok(results[0]);
	...
	public async Task<Outcome<IReadOnlyList<TView>>> ListAsync(Expression<Func<TView, bool>> predicate,
		CancellationToken cancellationToken = default)
	{
		var context = await factory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false);
		await using var contextDisposable = context.ConfigureAwait(false);
		IReadOnlyList<TView> list = await Query(context, predicate).Select(_selector).ToListAsync(cancellationToken)
			.ConfigureAwait(false);
		return Outcome<IReadOnlyList<TView>>.Ok(list);
	}

	public async Task<Outcome<IReadOnlyList<TProjection>>> ListAsync<TProjection>(
		Expression<Func<TView, bool>> predicate, Expression<Func<TView, TProjection>> projection,
		CancellationToken cancellationToken = default)
		where TProjection : notnull
	{
		var context = await factory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false);
		await using var contextDisposable = context.ConfigureAwait(false);
		IReadOnlyList<TProjection> list = await Query(context, predicate).Select(_selector).Select(projection)
			.ToListAsync(cancellationToken).ConfigureAwait(false);
		return Outcome<IReadOnlyList<TProjection>>.Ok(list);
	}

	public async Task<Outcome<TView>> SingleAsync(Expression<Func<TView, bool>> predicate,
		CancellationToken cancellationToken = default)
	{
		var context = await factory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false);
		await using var contextDisposable = context.ConfigureAwait(false);
		...
		var candidates = await Query(context, predicate).Select(_selector).Take(2).ToListAsync(cancellationToken)
			.ConfigureAwait(false);
	...
	public async Task<Outcome<TProjection>> SingleAsync<TProjection>(Expression<Func<TView, bool>> predicate,
		Expression<Func<TView, TProjection>> projection, CancellationToken cancellationToken = default)
		where TProjection : notnull
	{
		var context = await factory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false);
		...
		var candidates = await Query(context, predicate).Select(_selector).Select(projection).Take(2)
			.ToListAsync(cancellationToken).ConfigureAwait(false);
		return candidates.Count switch
		{
	...
	[RequiresUnreferencedCode(
		"Delegates to KeyPredicate.For, which resolves the entity's primary-key CLR property (and, for a non-Guid key, its Guid constructor) via runtime reflection; safe under the well-and-wire mirror law (spec §5.1) but not statically provable to the trimmer.")]

...
sealed record WellMap
{
	...
	public required
		FrozenDictionary<string, (PropertyInfo Navigation, FrozenDictionary<string, PropertyInfo> ElementMembers)>
		PromotedCollections
	{ get; init; }
	...
	[RequiresUnreferencedCode(
		"Reflects over TEntity's and TView's public properties by name to build the promotion map; the mirror law (well-and-wire spec §4.2) guarantees the shapes match at runtime, but the trimmer cannot see that statically.")]
	public static WellMap For<TEntity, TView>() where TEntity : IViewBearer<TView>
	{
		...
		var viewProperty =
			entityProps.Single(p => p.Name == nameof(IViewBearer<>.View) && p.PropertyType == typeof(TView));
		...
		{
			var entityMatch = entityProps.FirstOrDefault(p =>
				p.Name == viewProp.Name && p.GetCustomAttribute<NotProjectedAttribute>() is null);
			...
			var (viewElement, entityElement) =
				(ElementType(viewProp.PropertyType), ElementType(entityMatch.PropertyType));
	...
	[RequiresUnreferencedCode(
		"Reflects over the element type's interfaces to detect IEnumerable<T> shapes; safe under the mirror law but not statically provable to the trimmer.")]

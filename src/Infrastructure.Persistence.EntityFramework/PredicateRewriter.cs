...
{
	public static Expression<Func<TEntity, bool>> Rewrite<TEntity, TView>(Expression<Func<TView, bool>> predicate,
		WellMap map)
	{
	...
	sealed class Visitor(ParameterExpression viewParameter, ParameterExpression entityParameter, WellMap map)
		: ExpressionVisitor
	{
		protected override Expression VisitMember(MemberExpression node)
		{
			...
			return Expression.Property(Expression.Property(entityParameter, map.ViewProperty),
		...
		[UnconditionalSuppressMessage("Trimming", "IL2075",
			Justification =
				"Navigation.PropertyType's element type is resolved via WellMap.For, which already validated the mirror-law shape.")]
		[UnconditionalSuppressMessage("AOT", "IL3050",
			Justification =
				"Building the rewritten Any(...) call requires runtime Expression construction; this rewriter is not meant to run in an AOT-published context (EF's own query pipeline is not AOT-compatible either).")]
		protected override Expression VisitMethodCall(MethodCallExpression node)
		{
			...
			{
				var elementType = collection.Navigation.PropertyType.GetInterfaces()
					.Append(collection.Navigation.PropertyType)
					.First(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IEnumerable<>))
					.GetGenericArguments()[0];
				var elementParameter = Expression.Parameter(elementType, inner.Parameters[0].Name);
	...
	sealed class ElementVisitor(
		ParameterExpression viewElement,
		ParameterExpression entityElement,
		IReadOnlyDictionary<string, PropertyInfo> members) : ExpressionVisitor
	{
		protected override Expression VisitMember(MemberExpression node) =>

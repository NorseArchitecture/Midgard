...
static class KeyPredicate
{
	[RequiresUnreferencedCode(
		"Delegates to Cache<TEntity>.FactoryFor, which resolves the EF model's primary-key CLR property (and, for a non-Guid key, its Guid constructor) via runtime reflection on first use; not statically provable to the trimmer.")]
	...
	static class Cache<TEntity> where TEntity : class
	{
		...
		[RequiresUnreferencedCode(
			"Delegates to Build on first use, which resolves the EF model's primary-key CLR property (and, for a non-Guid key, its Guid constructor) via runtime reflection; not statically provable to the trimmer.")]
		public static Func<Guid, Expression<Func<TEntity, bool>>> FactoryFor(DbContext context) =>
			_factory ??= Build(context);

		[RequiresUnreferencedCode(
			"Resolves the EF model's primary-key CLR property, and, for a non-Guid key type, that type's public single-Guid constructor, both via runtime reflection over a Type discovered from the model at first use (well-and-wire spec §5.1) — not statically provable to the trimmer.")]
		static Func<Guid, Expression<Func<TEntity, bool>>> Build(DbContext context)
		{
			var primaryKey = context.Model.FindEntityType(typeof(TEntity))?.FindPrimaryKey()
				?? throw new InvalidOperationException(
					$"'{typeof(TEntity)}' has no primary key registered in the EF model.");
			if (primaryKey.Properties is not [IProperty keyProperty])
				throw new InvalidOperationException(
					$"'{typeof(TEntity)}' has a composite primary key; well-and-wire identity lookup requires a single-column key.");
			var keyPropertyInfo = keyProperty.PropertyInfo
				?? throw new InvalidOperationException(
					$"'{typeof(TEntity)}'s primary key '{keyProperty.Name}' is a shadow property; well-and-wire identity lookup requires a declared CLR property.");
			...
			if (keyPropertyInfo.PropertyType == typeof(Guid))
				return id =>
					Expression.Lambda<Func<TEntity, bool>>(Expression.Equal(keyAccess, Expression.Constant(id)),
						entity);

			var guidConstructor = keyPropertyInfo.PropertyType.GetConstructor([typeof(Guid)])
				?? throw new InvalidOperationException(
					$"Key type '{keyPropertyInfo.PropertyType}' is neither Guid nor exposes a public constructor accepting a single Guid; well-and-wire identity lookup cannot convert the incoming id.");
			return id =>
				Expression.Lambda<Func<TEntity, bool>>(
					Expression.Equal(keyAccess, Expression.New(guidConstructor, Expression.Constant(id))), entity);
		}
	}
}

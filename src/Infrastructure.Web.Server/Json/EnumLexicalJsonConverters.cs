...
/// <param name="xmlOptions">Carries the platform's single <see cref="NorseXmlOptions.CaseStyle"/> — the JSON and XML channels never diverge on which case style is live.</param>
public sealed class EnumLexicalJsonConverterFactory(EnumNameRegistry registry, NorseXmlOptions xmlOptions) : JsonConverterFactory
{
	/// <inheritdoc/>
	public override bool CanConvert(Type typeToConvert) => typeToConvert.IsEnum;

	/// <inheritdoc/>
	[UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "Same finite, closure-bounded posture as ResultJsonConverterFactory.CreateConverter: MakeGenericType closes over an enum type the generated registry already carries a table for, never an unbounded reflection surface.")]
	[UnconditionalSuppressMessage("Trimming", "IL2055", Justification = "MakeGenericType(typeToConvert) closes PlainEnumJsonConverter<> over an enum the generated registry already carries a table for — a registry-bounded set, never an unbounded reflection surface.")]
	[UnconditionalSuppressMessage("Trimming", "IL2071", Justification = "TEnum's struct-implying 'unmanaged, Enum' constraint carries an implicit PublicParameterlessConstructor annotation; every enum type trivially satisfies it (value types always have one), so the annotation mismatch on the unattributed typeToConvert parameter is a false positive here.")]
	[UnconditionalSuppressMessage("AotAnalysis", "IL3050", Justification = "Same posture as ResultJsonConverterFactory.CreateConverter: composition-root wiring over a doctrinally finite, registry-bounded set of enum types; AOT source-generation is a future increment.")]
	public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
	{
		if (!registry.TryGet(typeToConvert, out var table))
			throw new NotSupportedException($"no generated name table for enum '{typeToConvert.Name}' — an enum outside every facade closure has no text wire law");
		return (JsonConverter)Activator.CreateInstance(typeof(PlainEnumJsonConverter<>).MakeGenericType(typeToConvert), table, (int)xmlOptions.CaseStyle)!;
	}
}
...
/// <param name="styleIndex">The active <see cref="XmlCaseStyle"/>, as its ordinal column index into <paramref name="table"/>.</param>
public sealed class PlainEnumJsonConverter<TEnum>(EnumNameTable table, int styleIndex) : JsonConverter<TEnum> where TEnum : unmanaged, Enum
{
	/// <inheritdoc/>
...
/// <param name="registry">The generated per-enum name-table registry — the host passes the same instance <c>AddNorseJson</c> registers.</param>
/// <param name="xmlOptions">Carries the platform's single <see cref="NorseXmlOptions.CaseStyle"/> — the JSON and XML channels never diverge on which case style is live.</param>
public sealed class ResultEnumJsonConverterFactory(EnumNameRegistry registry, NorseXmlOptions xmlOptions) : JsonConverterFactory
{
	/// <inheritdoc/>
	...
	/// <inheritdoc/>
	[UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "Same finite, closure-bounded posture as ResultJsonConverterFactory.CreateConverter: MakeGenericType closes over an enum type the generated registry already carries a table for, never an unbounded reflection surface.")]
	[UnconditionalSuppressMessage("Trimming", "IL2055", Justification = "MakeGenericType(enumType) closes ResultEnumJsonConverter<>/NullableResultEnumJsonConverter<> over an enum the generated registry already carries a table for — a registry-bounded set, never an unbounded reflection surface.")]
	[UnconditionalSuppressMessage("Trimming", "IL2071", Justification = "TEnum's struct-implying 'unmanaged, Enum' constraint carries an implicit PublicParameterlessConstructor annotation; every enum type trivially satisfies it (value types always have one), so the annotation mismatch on the unattributed enumType local is a false positive here.")]
	[UnconditionalSuppressMessage("AotAnalysis", "IL3050", Justification = "Same posture as ResultJsonConverterFactory.CreateConverter: composition-root wiring over a doctrinally finite, registry-bounded set of enum types; AOT source-generation is a future increment.")]
	public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
	{
		if (typeToConvert.GetGenericTypeDefinition() == typeof(Result<>))
		{
			...
			return (JsonConverter)Activator.CreateInstance(typeof(ResultEnumJsonConverter<>).MakeGenericType(enumType), table, (int)xmlOptions.CaseStyle)!;
		}
		...
		return (JsonConverter)Activator.CreateInstance(typeof(NullableResultEnumJsonConverter<>).MakeGenericType(nullableEnumType), nullableTable, (int)xmlOptions.CaseStyle)!;
	}

	EnumNameTable Resolve(Type enumType) =>
		registry.TryGet(enumType, out var table) ?
			table :
			throw new NotSupportedException($"no generated name table for enum '{enumType.Name}' — an enum outside every facade closure has no text wire law");
...
/// <param name="table">The generated name table for <typeparamref name="TEnum"/>.</param>
/// <param name="styleIndex">The active <see cref="XmlCaseStyle"/>, as its ordinal column index into <paramref name="table"/>.</param>
public sealed class ResultEnumJsonConverter<TEnum>(EnumNameTable table, int styleIndex) : JsonConverter<Result<TEnum>> where TEnum : unmanaged, Enum
{
	/// <inheritdoc/>
	...
	internal static Result<TEnum> ReadPresent(ref Utf8JsonReader reader, EnumNameTable table, int styleIndex) =>
		reader.TokenType switch
		{
			...
			JsonTokenType.Number => new Failure(ParseFailure.Malformed, ReadNumberInvariant(ref reader), table.TypeName),
			JsonTokenType.True or JsonTokenType.False => new Failure(ParseFailure.Malformed, reader.GetBoolean() ? "true" : "false", table.TypeName),
...
/// <param name="table">The generated name table for <typeparamref name="TEnum"/>.</param>
/// <param name="styleIndex">The active <see cref="XmlCaseStyle"/>, as its ordinal column index into <paramref name="table"/>.</param>
public sealed class NullableResultEnumJsonConverter<TEnum>(EnumNameTable table, int styleIndex) : JsonConverter<Result<TEnum>?> where TEnum : unmanaged, Enum
{
	/// <inheritdoc/>
	public override Result<TEnum>? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
		reader.TokenType == JsonTokenType.Null ? null : ResultEnumJsonConverter<TEnum>.ReadPresent(ref reader, table, styleIndex);

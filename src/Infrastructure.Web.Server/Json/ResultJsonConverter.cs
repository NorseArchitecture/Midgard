...
public sealed class ResultJsonConverter<T> : JsonConverter<Result<T>> where T : notnull, ISpanParsable<T>
{
	...
	internal static Result<T> ReadPresent(ref Utf8JsonReader reader) =>
		...
		{
			JsonTokenType.String => typeof(T) == typeof(string) ?
				ReadStringDirect(ref reader) :
				Parser.ParseRequired<T>(reader.GetString() ?? string.Empty, CultureInfo.InvariantCulture),
			JsonTokenType.Number => Parser.ParseRequired<T>(ReadNumberInvariant(ref reader),
				CultureInfo.InvariantCulture),
			JsonTokenType.True or JsonTokenType.False => Parser.ParseRequired<T>(reader.GetBoolean() ?
				"true" :
	...
	[UnconditionalSuppressMessage("Trimming", "IL2026",
		Justification =
			"Same finite scalar taxonomy as ResultJsonConverterFactory.CreateConverter (spec §7, ~13 types, all ISpanParsable<T>); no unbounded reflection surface to trim.")]
	[UnconditionalSuppressMessage("AotAnalysis", "IL3050",
		Justification =
			"Same posture as ResultJsonConverterFactory.CreateConverter: the closed scalar taxonomy is doctrinally finite; AOT source-generation for it is a future increment.")]
...
public sealed class NullableResultJsonConverter<T> : JsonConverter<Result<T>?> where T : notnull, ISpanParsable<T>
{
	...
	public override Result<T>? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
		reader.TokenType == JsonTokenType.Null ?
			null :
			ResultJsonConverter<T>.ReadPresent(ref reader);
	...
	///     <paramref name="value" /> is present and holds a <see cref="Failure" /> or
	///     defaulted <see cref="Result{T}" />.
	/// </exception>
	public override void Write(Utf8JsonWriter writer, Result<T>? value, JsonSerializerOptions options)
	{
		if (!value.HasValue)

...
public sealed class ResultJsonConverterFactory : JsonConverterFactory
{
	...
	[UnconditionalSuppressMessage("AotAnalysis", "IL3050",
		Justification =
			"The scalar taxonomy Result<T> closes over (ISpanParsable<T> + the PII rows) is a doctrinally finite set (spec §7); AOT source-generation for it is a future increment.")]
	[UnconditionalSuppressMessage("Trimming", "IL2055",
		Justification =
			"Same finite-taxonomy posture as IL3050 above: every closed Result<T> converter shape is reachable only over the doctrinally finite scalar set, all of whose converter types are declared in this assembly.")]
	public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
	{
		if (typeToConvert.GetGenericTypeDefinition() == typeof(Result<>))
		{
			...
			return (JsonConverter)Activator.CreateInstance(
				(IsPiiScalar(valueType) ?
					typeof(PiiResultJsonConverter<>) :
		...
		return (JsonConverter)Activator.CreateInstance(
			(IsPiiScalar(nullableValueType) ?
				typeof(NullablePiiResultJsonConverter<>) :
				typeof(NullableResultJsonConverter<>)).MakeGenericType(nullableValueType))!;
	}
	...
	[UnconditionalSuppressMessage("Trimming", "IL2070",
		Justification =
			"The PII scalars are concrete structs referenced directly by the contract types that carry them; their interface lists survive trimming with the types themselves.")]
	static bool IsPiiScalar(Type type) =>
		Array.Exists(type.GetInterfaces(),
			static i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(Norse.Primitives.Pii.IPiiScalar<>));
}

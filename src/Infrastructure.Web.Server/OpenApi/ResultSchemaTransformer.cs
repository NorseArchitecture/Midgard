...
/// <see cref="ScalarTaxonomy"/>'s closed static table and its generic enum branch — never the union's
...
/// transformer also table-drives every raw closed-taxonomy scalar member it finds, marking it
/// <c>readOnly</c> — request members get <c>writeOnly</c>, response members get <c>readOnly</c>, both
/// from the same one table (spec §12). Scoped to <c>[DataContract]</c> types only
/// (<see cref="ScalarTaxonomy.IsFutharkContract"/>) — see that method's remarks for why touching a
/// schema outside that gate is not just out of scope but empirically unsafe.
/// </summary>
public sealed class ResultSchemaTransformer(NorseXmlOptions options) : IOpenApiSchemaTransformer
{
	...
	public Task TransformAsync(OpenApiSchema schema, OpenApiSchemaTransformerContext context, CancellationToken cancellationToken)
	{
		...
		foreach (var property in context.JsonTypeInfo.Properties)
		{
			...
			if (TryBuildScalarSchema(property.PropertyType, out var raw))
			{
				raw.ReadOnly = true;
				schema.Properties ??= new Dictionary<string, IOpenApiSchema>();
	...
	/// <summary>The fixed BCL table first, the generic enum branch second — the two rows of §7's taxonomy this transformer's closed-table mechanism has to offer.</summary>
	bool TryBuildScalarSchema(Type clrType, out OpenApiSchema schema) =>
		ScalarTaxonomy.TryBuildSchema(clrType, out schema) || ScalarTaxonomy.TryBuildEnumSchema(clrType, _options.CaseStyle, out schema);
}

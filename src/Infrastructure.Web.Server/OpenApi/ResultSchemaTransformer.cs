...
/// </summary>
public sealed class ResultSchemaTransformer(EnumNameRegistry registry, NorseXmlOptions options) : IOpenApiSchemaTransformer
{
	...
	/// </summary>
	/// <exception cref="NotSupportedException"><paramref name="clrType"/> is an enum with no table registered in <see cref="EnumNameRegistry"/>.</exception>
	bool TryBuildScalarSchema(Type clrType, out OpenApiSchema schema)
	{
		...
		if (!_registry.TryGet(clrType, out var table))
			throw new NotSupportedException($"no generated name table for enum '{clrType.Name}' — an enum outside every facade closure has no text wire law");

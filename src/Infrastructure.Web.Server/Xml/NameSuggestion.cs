...
{
	/// <summary>
	///     The maximum edit distance a suggestion is offered within — beyond this, the names are unrelated, and no
	...
	/// <summary>
	///     Case-insensitive Levenshtein edit distance between <paramref name="a" /> and <paramref name="b" /> — classic
	///     single-row dynamic-programming form, O(a.Length * b.Length) time, O(min(a.Length, b.Length)) space.
	/// </summary>
	static int Distance(string a, string b)
	{
		...
		for (var i = 1; i <= a.Length; i++)
		{
			...
			{
				var cost = char.ToUpperInvariant(a[i - 1]) == char.ToUpperInvariant(b[j - 1]) ?
					0 :
					1;

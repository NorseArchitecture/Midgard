...
public sealed class XmlReadContext
{
	...
	/// <summary>Renders the current path with <paramref name="attributeName"/> appended as an attribute, e.g. <c>Policy/Coverage[2]/@limit</c>.</summary>
	...
	public string CurrentPath =>
		string.Join('/', _segments.Select(static segment =>
			segment.Index > 0 ? $"{segment.Name}[{segment.Index}]" : segment.Name));
	...
	/// <summary>Records a scalar conversion failure at <paramref name="attributeName"/> under the current path, rendered through <see cref="FailureDetail.Render"/>.</summary>

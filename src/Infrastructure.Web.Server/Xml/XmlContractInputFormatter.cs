...
sealed class XmlContractInputFormatter : TextInputFormatter
{
	...
	public override async Task<InputFormatterResult> ReadRequestBodyAsync(InputFormatterContext context, Encoding encoding)
	{
		...
		if (!_registry.TryGet(context.ModelType, out var shape))
			throw new InvalidOperationException($"no XML shape is registered for '{context.ModelType}' — a type outside the exposed closure is not serializable");

		byte[] body;
		using (MemoryStream buffer = new())
		{
			await context.HttpContext.Request.Body.CopyToAsync(buffer, context.HttpContext.RequestAborted).ConfigureAwait(false);
			body = buffer.ToArray();
		}
	...
	/// <summary>True when the current element or any of its attributes carries a resolved namespace URI — covers both a default <c>xmlns="..."</c> and a prefixed <c>xmlns:p="..."</c>/<c>p:name</c> form; Futhark documents carry no namespaces, ever (spec §6.6, §8.1).</summary>

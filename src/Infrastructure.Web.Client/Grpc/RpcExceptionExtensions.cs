		...
		public Problem DecodeProblem()
		{
			...
			foreach (var detail in richStatus.Details)
			{
				if (detail.Is(ErrorInfo.Descriptor) && detail.TryUnpack<ErrorInfo>(out var errorInfo) &&
					Enum.TryParse<ErrorCategory>(errorInfo.Reason, out var parsed))
				{
					category = parsed;
					if (errorInfo.Metadata.TryGetValue("receipt", out var receiptId) &&
						...
						DateTimeOffset.TryParse(severedAt, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind,
							out var parsedSeveredAt))
					{
				...
				else if (detail.Is(BadRequest.Descriptor) && detail.TryUnpack<BadRequest>(out var badRequest))
				{
					errors = badRequest.FieldViolations
						...
						.ToDictionary(group => group.Key,
							group => group.Select(violation => violation.Description).ToArray());
				}
				else if (detail.Is(DebugInfo.Descriptor) && detail.TryUnpack<DebugInfo>(out var debugInfo) &&
					Guid.TryParse(debugInfo.Detail, out var parsedCorrelationId))
				{

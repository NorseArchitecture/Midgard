...
sealed class ResultSerializer<T> : ISerializer<Result<T>>, ISerializer<Result<T>?> where T : notnull, ISpanParsable<T>
{
	...
	/// <summary>Native per-type write dispatch, branch-for-branch the write counterpart of <see cref="ReadScalar"/>. Never called for <see cref="DateTimeOffset"/> — <see cref="Write"/> routes that one through its own "O" string branch before ever reaching here.</summary>
	static void WriteScalar(ref ProtoWriter.State state, T value)
	{
		if (typeof(T) == typeof(bool))
		{ state.WriteBoolean(Unsafe.As<T, bool>(ref value)); return; }
		if (typeof(T) == typeof(byte))
		{ state.WriteInt32(Unsafe.As<T, byte>(ref value)); return; }
		if (typeof(T) == typeof(sbyte))
		{ state.WriteInt32(Unsafe.As<T, sbyte>(ref value)); return; }
		if (typeof(T) == typeof(short))
		{ state.WriteInt32(Unsafe.As<T, short>(ref value)); return; }
		if (typeof(T) == typeof(ushort))
		{ state.WriteInt32(Unsafe.As<T, ushort>(ref value)); return; }
		if (typeof(T) == typeof(int))
		...
		if (typeof(T) == typeof(char))
		{ state.WriteUInt16((ushort)Unsafe.As<T, char>(ref value)); return; }
		if (typeof(T) == typeof(string))

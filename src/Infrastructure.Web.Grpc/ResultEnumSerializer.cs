...
///     The contract's enum type — user-declared, so the set is open; registration is discovery-driven
///     (see <see cref="ResultSerializers" />).
/// </typeparam>
sealed class ResultEnumSerializer<TEnum> : ISerializer<Result<TEnum>>, ISerializer<Result<TEnum>?>
	...
	public void Write(ref ProtoWriter.State state, Result<TEnum> value)
	{
		...
		if (!IsDefined(success.Value))
			throw new InvalidOperationException(
				$"'{success.Value}' is an undefined value of '{typeof(TEnum)}' and is illegal to write.");

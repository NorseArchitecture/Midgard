...
public static class ResultSerializers
{
	...
	public static void Register(RuntimeTypeModel model) =>
		model.EnsureRegistered(typeof(ResultSerializers), () =>
		{
			...
			model.Add(typeof(DateTimeOffset), applyDefaultBehaviour: false).SerializerType =
				typeof(DateTimeOffsetSerializer);
			...
			RegisterPiiScalar<EmailAddress>(model);
			RegisterPiiScalar<PersonalName>(model);
			RegisterPiiScalar<PhoneNumber>(model);
			RegisterPiiScalar<BirthDate>(model);
		});
	...
	static void RegisterPiiScalar<T>(RuntimeTypeModel model) where T : struct, IPiiScalar<T> =>
		model.EnsureRegistered(typeof(Result<T>), () =>
			model.Add(typeof(Result<T>), applyDefaultBehaviour: false).SerializerType = typeof(PiiResultSerializer<T>));

	[UnconditionalSuppressMessage("Trimming", "IL2055",
		Justification =
			"ResultEnumSerializer<TEnum> is fully generic over enum types with no member dependencies beyond the enum itself; contract types that reach this sweep are already rooted by the model registration that triggered it.")]
	[UnconditionalSuppressMessage("AotAnalysis", "IL3050",
		Justification =
			"Same posture as ResultJsonConverterFactory: the enum set is contract-declared and discovery-driven; AOT source-generation for it is a future increment.")]

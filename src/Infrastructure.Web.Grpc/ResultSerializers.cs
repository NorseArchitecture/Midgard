...
/// registration mechanism. Wire form: the naked <c>T</c>, presence-tracked — a successful
/// <see cref="Result{T}"/> writes <c>T</c> directly; a failed or default <see cref="Result{T}"/> is
/// illegal to write and throws <see cref="InvalidOperationException"/>; an absent field on read
/// leaves the member at <c>default(Result{T})</c> — protobuf-net's native behavior for any field
/// whose tag never appears on the wire, since deserialize only ever invokes a field's serializer for
/// tags it actually finds — which <c>Infrastructure.Web.Server</c>'s <c>ResultRules</c> validation
/// catches downstream (spec §9.3). <c>Result{T}?</c> (<see cref="Nullable{T}"/> of
/// <see cref="Result{T}"/>) needs no separate registration: protobuf-net's built-in
/// <see cref="Nullable{T}"/> handling already skips the write when null and leaves null on an absent
/// read; a <c>Failure</c>-cased <see cref="Result{T}"/> wrapped in the nullable still reaches the
/// same serializer's <c>Write</c> and still throws.
/// </summary>
/// <remarks>

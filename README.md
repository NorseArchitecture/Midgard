...
**`Norse.Infrastructure.Migrations` is live** — `MigrationRunnerService` and `AddNorseMigrationsRunner()` shipped as the runner every Norse migrations service calls through, part of the platform-wide migrations framework proven end to end across six realms (the full story is on [Bifröst's README](https://github.com/NorseArchitecture/Bifrost#readme)). **`Infrastructure.Web.Server`/`.Web.Client` are also live** — the gRPC mediator transport and the `IDeferredSignIn` implementation against Asgard's contract (Himinbjörg is the pending consumer). **`Infrastructure.Web.Grpc` is live** too — the shared wire-law project carrying `IdentifierSerializers` (CompatibilityLevel 300 per contract member, identifiers as bare 16-byte RFC 9562 `bytes` fields, `SequentialGuid`/`DeterministicGuid` custom scalar serializers), referenced by both `Web.Client` and `Web.Server` and registered by the generated client/server wiring — one of Midgard's two edges to Svartálfheim (the other is `Infrastructure.Backend`'s direct reference, below). **`Infrastructure.Components.Theme`/`.Theme.FluentUI` are live** too — the first slice of UI composition (app-wide theme bootstrapping). **`Infrastructure.Backend` is live** too — Midgard's shared server-side assembly (the mirror of Asgard's `Abstractions.Backend`), home to concerns both `{Realm}.Web.Server` and `{Realm}.Worker` consume; no per-functional-group packages. First resident, under `Serialization/`: the System.Text.Json machinery behind Asgard's format-agnostic serialization seam (`ISerializer`/`ISerializerProvider`, naming-strategy keyed, wired via `AddNorseSerialization()`); STJ stays caged here, inside the wire border, per NORSE070. The same folder carries `MaskedValueJsonConverterFactory`, the masked-serialization defense that makes `IMaskedValue` members mask on every Midgard JSON path. `Keys/` also lands here — `DevelopmentSubjectKeyStore`/`AddNorseDevelopmentKeys()`, a file-backed, dev-grade-only implementation of Asgard's `ISubjectKeyStore`/`ILookupKeyRing` key seam (keys rest unwrapped on local disk; never a production path). The dashboard-widget-composition half of UI composition remains unconverged, and the `DbContext` family and repository implementations are still unconverged. Design happens first for what's left: brainstorm → spec → plan, recorded in Glitnir's `docs/Midgard/`, before any further project is scaffolded here.
